...
plt.show()
'''
df.loc[:,'date']=[int(a[:4]) for a in df['date']]
#df.plot(kind='scatter',x='date',y='price')  #绘图
#plt.show()
...
print ("岭回归模型的准确率是",ridge.score(X_test, y_test))

#改进：xlj
svr = SVR(kernel='rbf', C=1000)
svr.fit(X_train,y_train)
y_predicted = svr.predict(X_test)
print ("svr回归模型的准确率是",svr.score(X_test, y_test))
...
model_gbr = GradientBoostingRegressor()
#对loss，min_samples_leaf，alpha三个参数值进行最优化网格搜索
parameters = {'loss': ['ls'],'min_samples_leaf': [1,2,3],'alpha': [0.5,0.6,0.7,0.8]}
#调用网格搜索模型进行最优化参数搜索
model_gs = GridSearchCV(estimator=model_gbr, param_grid=parameters, cv=5)
model_gs.fit(X_train,y_train)
#输出最优的模型评估值和模型参数值
...
#采用最优参数进行数据建模分析
#配置最优模型参数的模型
model = GradientBoostingRegressor(n_estimators=500,alpha=0.7,loss='ls',min_samples_leaf=2)
#调用最优模型参数进行训练
model.fit(X_train, y_train)
#使用最优模型进行模型预测
y_pred=model.predict(X_test)
